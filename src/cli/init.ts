...
const engine = new TrigramSearchEngine(adapter, {
    ...
    `;
    console.log(pc.white(configSnippet));

    // --- Save to File ---
    const savePrompt = new Confirm({
        name: 'save',
        message: pc.bold(pc.cyan('💾 Save these as files?')) + pc.dim(' (search-setup.sql and search-config.js)'),
        initial: true
    });
    
    const shouldSave = await savePrompt.run();
    if (shouldSave) {
        await fs.writeFile('search-setup.sql', sql.trim());
        await fs.writeFile('search-config.js', `// pg-smart-search configuration generated by CLI
const { TrigramSearchEngine, SearchTier } = require('pg-smart-search');
...
`);
        console.log(pc.green(`\n✅ Files saved: ${pc.bold('search-setup.sql')}, ${pc.bold('search-config.js')}`));
    }
